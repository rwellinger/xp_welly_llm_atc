// Catch2 unit tests for the VRP database loader. Pins the JSON-backed
// VRP set against the lookup API used by intent_parser
// (find_in_transcript) and the pattern-flow / ground-operations modules
// (get_pattern_direction). VRP data is profile-independent; EDNY
// (Friedrichshafen) exercises the EU/ICAO VRP-arrival flow.
...
// RAII guard: reloads the VRP database, then restores the previous
// profile so the test order does not contaminate neighbouring TUs.
struct VrpGuard {
    std::string saved_region;
    VrpGuard() : saved_region(settings::atc_profile()) {
...
// Like VrpGuard but also points XP_ATC_USER_PREFS_DIR at the fixtures
// directory so the loader picks up tests/fixtures/user_prefs/airport_vrps.json
// as a user override on top of the plugin defaults.
struct VrpUserOverrideGuard {
    std::string saved_region;
    bool had_env;
    std::string saved_env;
    VrpUserOverrideGuard() : saved_region(settings::atc_profile()) {
