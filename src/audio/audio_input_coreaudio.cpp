...
// CoreAudio HAL implementation of IAudioInput. Selected via the
// make_audio_input() factory at the bottom of this TU. Plugin-only —
// the CMake build gates this file behind `if(APPLE)`.
...
constexpr unsigned kDesiredSampleRate = 16000;
constexpr unsigned kBitsPerSample = 16;
constexpr unsigned kNumChannels = 1;

class CoreAudioInput : public IAudioInput {
public:
  CoreAudioInput() = default;
  ~CoreAudioInput() override { close(); }
  ...
  unsigned sample_rate_hz() const override { return actual_sample_rate_; }
  std::size_t buffer_samples() const override;
  float duration_seconds() const override;

private:
  static OSStatus render_trampoline(void *in_ref_con,
  ...
  OSStatus render(AudioUnitRenderActionFlags *io_action_flags,
                  const AudioTimeStamp *in_time_stamp, UInt32 in_number_frames);

  void log_default_input_device();

  AudioComponentInstance audio_unit_ = nullptr;
  unsigned actual_sample_rate_ = kDesiredSampleRate;
  std::vector<int16_t> buffer_;
  mutable std::mutex buffer_mutex_;
  std::atomic<bool> recording_{false};
  std::atomic<int> render_call_count_{0};
  bool initialized_ = false;
};

OSStatus CoreAudioInput::render_trampoline(
    void *in_ref_con, AudioUnitRenderActionFlags *io_action_flags,
    const AudioTimeStamp *in_time_stamp, UInt32 /*in_bus_number*/,
    UInt32 in_number_frames, AudioBufferList * /*io_data*/) {
  auto *self = static_cast<CoreAudioInput *>(in_ref_con);
  return self->render(io_action_flags, in_time_stamp, in_number_frames);
}

OSStatus CoreAudioInput::render(AudioUnitRenderActionFlags *io_action_flags,
                                const AudioTimeStamp *in_time_stamp,
                                UInt32 in_number_frames) {
...
void CoreAudioInput::log_default_input_device() {
...
bool CoreAudioInput::open() {
  ...
  callback.inputProc = &CoreAudioInput::render_trampoline;
...
void CoreAudioInput::start_recording() {
...
void CoreAudioInput::stop_recording() {
  recording_ = false;
  if (audio_unit_) {
    AudioOutputUnitStop(audio_unit_);
  }
...
std::vector<int16_t> CoreAudioInput::take_pcm() {
  std::lock_guard<std::mutex> lock(buffer_mutex_);
  return std::move(buffer_);
}

std::size_t CoreAudioInput::buffer_samples() const {
  std::lock_guard<std::mutex> lock(buffer_mutex_);
  return buffer_.size();
}

float CoreAudioInput::duration_seconds() const {
  std::lock_guard<std::mutex> lock(buffer_mutex_);
  if (buffer_.empty())
    return 0.0f;
