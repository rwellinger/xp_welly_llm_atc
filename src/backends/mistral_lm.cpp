...
namespace {
constexpr const char *kBackendTag = "LM-MISTRAL";
...
MistralLm::MistralLm(std::string api_key, std::string model,
                     std::string base_url)
    : api_key_(std::move(api_key)), model_(std::move(model)),
      base_url_(std::move(base_url)) {}

std::string MistralLm::respond(const std::string &system_prompt,
                               const std::string &user_text) {
  return call(system_prompt, user_text, /*json_mode=*/false);
}

std::string MistralLm::respond_constrained(const std::string &system_prompt,
                                           const std::string &user_text,
                                           const std::string &grammar_gbnf) {
  // GBNF is a llama.cpp construct; Mistral has no equivalent. The
  // caller's contract is "JSON-shaped output" — we honor that via
  // response_format=json_object and rely on the system_prompt to
  // describe the schema. The unused grammar is intentionally dropped.
  (void)grammar_gbnf;
